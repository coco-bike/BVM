...
namespace API.Controllers
{
    public class ValuesController : JsonController
    {
        ...
        private IRoleAdminService _roleService { get; set; }
        ...
        public IRoleAdminService UserUpFileAdminService
        {
            get
            {
        ...
        public JsonNetResult LogIn(string nickName, string pwd)
        {
            ...
            var user = UserService.GetList(t => t.NickName == nickName && t.Password == pwdMd5 && t.Status == 1).Select(t => new { t.ID, t.NickName, t.Count }).FirstOrDefault();
            if (user == null)
            {
                return JsonNetResult(ResultStatus.Fail);
            }
            int res = UserService.GetUpdate(t => t.NickName == nickName, t => new User() { Count = user.Count+1, LoginTime = DateTime.Now });
            if (res > 0)
        ...
        //        {
        //            //var regUser = this.UserService.Add(new User() { EMail = email, Pwd = EncryptionHelper.GetMd5Str(pwd), UName = "", BuildTime = DateTime.Now, Count = 0, State = 0, UpdateTime = DateTime.Now, RoleId = 1, OrganizeId = 1, LevelId = 1, FolderUrl = "/FileSave/YUN/" + nickName, LoginTime = DateTime.Now, YunUsedSpace = 0, NickName = nickName, Status = 1 });
        //            if (regUser != null)
        //            {
